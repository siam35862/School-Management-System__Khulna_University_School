...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['st_ID'], $_POST['subject_id'], $_POST['mark'])) {
    ...
    try {
        ...
        $conn->rollback();
        echo "<script>alert('Error: " . $e->getMessage() . "'); window.location.href = 'teacher.php';</script>";
    }
} else {
    echo "<script>alert('Invalid data submitted. Please ensure you selected a subject and entered a mark.'); window.location.href = 'teacher.php';</script>";
}
